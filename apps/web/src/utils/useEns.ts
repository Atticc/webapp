...
const useEns = (addressOrName: string | undefined) => {
  ...
  const probableAddress = addressOrName?.startsWith('0x') && addressOrName?.length === 42 ? addressOrName : undefined
  ...
  }, [probableName, probableAddress, resolveName, lookupAddress, getAvatarUrl, name])
